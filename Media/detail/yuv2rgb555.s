...
.globl	uyvy422_2_rgb555
.type uyvy422_2_rgb555, function

# UYVY, GCC compatibility, src/dst dimension support by Hello007

# void yuv420_to_rgb555
#  uint8_t *x_ptr
#  uint8_t *y_ptr
#  uint8_t *u_ptr
#  uint8_t *v_ptr
...
uyvy422_2_rgb555:
	...
	LDR	DstH, [r13,#10*4]		@ r8 = height
	LDR	DstW, [r13,#9*4]		@ r9 = width
    CMP DstH, YCtr
    MOVLT YCtr, DstH        @ if (dstH < srcH) srcH = dstH - use lower height coord
    SUBS StrideAddend, DstW, Width @ strideAddend = (dstW - srcW)
    MOVLT Width, DstW        @ if (dstW < srcW) srcW = dstW - use lower width coord
    MOVGE StrideAddend, #0
    LSL StrideAddend, StrideAddend, #2
	LDR	r4, =0x07C07C1F
	LDR	r5, =0x40040100
	MOV	Width, Width, LSR #1 @ width = (width / 2)
yloop1:
	MOV	XCtr, Width	@ xctr = width
xloop1:
	LDRB	U,[UYVYPtr], #1		@ r11 = u  = *uyvy_ptr++
    LDRB	Y0, [UYVYPtr], #1		@ r6  = y0 = *uyvy_ptr++
	LDRB	V,[UYVYPtr], #1		@ r12 = v  = *uyvy_ptr++
    LDRB	Y1, [UYVYPtr], #1		@ r6  = y1 = *uyvy_ptr++
	ADR	Table,u_table
	LDR	U,[Table,U,LSL #2]	@ r11 = u  = u_table[u]
	ADD	Table,Table,#1024		@ r14 = v_table
	LDR	V,[Table,V,LSL #2]	@ r12 = v  = v_table[v]
	ADD	Table,Table,#1024		@ r14 = y_table
	LDR	Y0, [Table,Y0, LSL #2]	@ r6  = y0 = y_table[y0]
    LDR	Y1, [Table, Y1, LSL #2]	@ r6  = y1 = y_table[y0]
	ADD	r11,U,V		@ r11 = uv = u+v

	ADD	r12,r11,r5, LSR #6	@ (dither 1/4)
	ADD	Y0, Y0, r12		@ r6  = y2 + uv
	ADD	Y0, Y0, r5, LSR #7	@ (dither 3/4)
	TST	Y0, r5
	BNE	fix101
return101:
	ADD	r12,r11,r5, LSR #6	@ (dither 1/4)
    ADD	Y1, Y1, r12		@ r6  = y1 + uv
    ADD	Y1, Y1, r5, LSR #7	@ (dither 3/4)
	TST	Y1, r5
    BNE fix102
return102:
	AND	Y0, r4, Y0, LSR #3
...
fix102:
	@ r7 and r6 are the values, at least one of which has overflowed
	@ r12 = r7 & mask = .s......s......s......
	AND	r12, r7, r5		@ r12 = .S......S......S......
	SUB	r12,r12,r12,LSR #8	@ r12 = ..SSSSSS..SSSSS.SSSSSS
	ORR	r7, r7, r12		@ r6 |= ..SSSSSS..SSSSS.SSSSSS
	BIC	r12,r5, r7, LSR #1	@ r12 = .o......o......o......
	ADD	r7, r7, r12,LSR #8	@ r6  = fixed value
	B	return102
...
.globl	uyvy422_2_rgb555_interwork
.type uyvy422_2_rgb555_interwork, function
.force_thumb
.thumb_func
uyvy422_2_rgb555_interwork:
    PUSH {R4}
    LDR R4, =uyvy422_2_rgb555
    MOV R12, R4
    POP {R4}
    BX R12
